install_conda_env.sh#!/bin/bash
set -e  # Stop on error

install_ucsc_tools_369() {
  # takes in conda env name and find conda bin
  CONDA_BIN=$(micromamba run -n $1 bash -c "echo \$(dirname \$(which python))")
...
CONDA_BIN=$(micromamba run -n encd-chip-spp bash -c "echo \$(dirname \$(which python))")
...
micromamba run -n encd-chip-spp bash -c \
